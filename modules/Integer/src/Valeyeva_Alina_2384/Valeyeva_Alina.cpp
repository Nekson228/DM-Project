...
Integer &Integer::operator+(const Integer &other) const {
    ...
    if ((other.isPositive() == 2 ||other.isPositive() == 0) && (this->isPositive() == 2 || this->isPositive() == 0)){
        result = &(number_ + other.number_); //  складываю два натур числа без модуля, т.к. положительные
    }
    ...
    Integer *result_integer = new Integer(*result);
    if (flag_MUL_ZM_Z == 1){
        result_integer->negative(); // меняем знак на минус, домножая на (-1)
    }
...
//// ADD_ZZ_Z
//Integer &Integer::operator+(const Integer &other) const {
//    int flag_MUL_ZM_Z = 0; // для определения знака (MUL_ZM_Z)
////    Natural number_other = other.abs().toNatural(); // получаю абсолютную часть второго числа (ABS_Z_Z)
//    Natural number_other = other.abs().number_; // получаю абсолютную часть второго числа (ABS_Z_Z)
//
//    digit my_cmp = Natural::cmp(number_, number_other); // сравниваню два натуральных числа (COM_NN_D)
//    // если числа положительные или равны 0 (POZ_Z_D)
//    if ((other.isPositive() == 2 ||other.isPositive() == 0) && (this->isPositive() == 2 || this->isPositive() == 0)){
//        Natural result = number_ + other.number_; //  складываю два натур числа без модуля, т.к. положительные
//    }
//    // если оба числа отрицательные
//    else if (other.isPositive() == 1 && this->isPositive() == 1){
//        Natural result = number_ + number_other; // складываю два натуральных числа(ADD_NN_N)
//        flag_MUL_ZM_Z = 1; // нужно поменять знак у моего возвращаемого целого числа
//    }
//    // Если одно число положительное, а другое отрицательное
//    else{
//        // если первое > второго или они равны
//        if (my_cmp == 2 || my_cmp == 0){
//            Natural result = number_ - number_other; // (SUB_NN_N)
//        }
//        // если первое < второго (my_cmp == 1)
//        else{
//            Natural result = number_other - number_; // вычитаем из второго первое (SUB_NN_N)
//            flag_MUL_ZM_Z = 1; // но впоследствии меняем знак у ответа
//        }
//    }
//    Integer *result_integer = new Integer(result);
//    if (flag_MUL_ZM_Z == 1){
//        result_integer->sign_ = true;
//    }
//    return *result_integer;
//}
